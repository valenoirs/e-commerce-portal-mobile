...
const ProfileScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    AsyncStorage.getItem("user").then((value: any) => {
      ...
      axios
        .get(`${config.api_host}/api/order?userId=${JSON.parse(value).id}`)
        .then((response) => {
          ...
          {modalOrderContent ? (
            <View style={styles.modalOrderContent}>
              ...
              <View style={styles.confirmOrderContainer}>
                {modalOrderContent.status === "Menunggu Pembayaran" ? (
                  <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  userContainer: { padding: 30, borderBottomWidth: 1, borderColor: "#cccccc" },
  userName: {
    color: "green",
    fontWeight: "600",
    fontSize: 22,
  ...
  modalTokoInvoice: { fontSize: 17, color: "gray", fontWeight: "500" },
  modalTokoStatus: {
    color: "gray",
    fontSize: 14,
  },
  ...
  tokoContentInvoice: { color: "grey", fontWeight: "500", fontSize: 14 },
  tokoContentToko: {
    color: "grey",
    fontWeight: "400",
    fontSize: 13,
